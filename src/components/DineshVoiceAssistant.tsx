...
import { Mic, MicOff, X, Send, Volume2, VolumeX, MessageCircle, HeadphonesIcon, Settings, Languages, User, Sliders } from "lucide-react";
...
export function DineshVoiceAssistant({ userName, userId }: DineshProps) {
    ...
    const speak = (text: string) => {
        ...
        if (!settings.voiceName && synthRef.current) {
            ...
            const preferredVoice = voices.find(v =>
                v.lang.includes(settings.language.split('-')[0]) &&
    ...
    const processQuery = async (query: string): Promise<string> => {
        // Step 1: Detect language if not explicitly locked or for new interaction
        const detectedLang = detectLanguage(query);
        const lang = settings.language; // We'll stick to current setting but could auto-switch
        const lowerQuery = query.toLowerCase();

        // Step 2: Check for AI service recommendations if it's a problem query
        if (isProblemQuery(query)) {
            const recs = getServiceRecommendations(query);
            if (recs.length > 0) {
                sounds.playSuccess();
                return generateRecommendationResponse(query, recs);
            }
        }
        ...
        // Homepage
        if (matchesCategory(query, "home", settings.language)) {
            setTimeout(() => router.push("/"), 1000);
            return languageResponses[settings.language].home || "Taking you to the homepage now.";
        }

        // Booking
        if (matchesCategory(query, "booking", settings.language)) {
            setTimeout(() => router.push("/booking"), 1000);
            return languageResponses[settings.language].bookingResponse;
        }

        // Services
        if (matchesCategory(query, "services", settings.language) && !lowerQuery.includes("track")) {
            setTimeout(() => router.push("/services"), 1000);
            return languageResponses[settings.language].servicesResponse;
        }
        ...
        if (lowerQuery.includes("gallery") || lowerQuery.includes("photos") || lowerQuery.includes("pictures") || lowerQuery.includes("before after")) {
            ...
            return "Opening our gallery to show you amazing before and after transformations of vehicles we've worked on.";
        }
        ...
        if (lowerQuery.includes("about") || lowerQuery.includes("who are you") || lowerQuery.includes("your company")) {
            ...
            return "Let me tell you about Shashti Karz. We're a premium car detailing service with years of experience in automotive care.";
        }
        ...
        if (lowerQuery.includes("contact") || lowerQuery.includes("reach you") || lowerQuery.includes("phone number")) {
            ...
            return "Here's our contact information. You can call us, send a message, or visit our location.";
        }
        ...
        if (lowerQuery.includes("virtual tour") || lowerQuery.includes("360") || lowerQuery.includes("tour")) {
            ...
            return "Let me show you our 360-degree virtual tour! You can explore our facility virtually.";
        }
        ...
        if (lowerQuery.includes("my booking") || lowerQuery.includes("track") || lowerQuery.includes("order status") || lowerQuery.includes("booking history")) {
            ...
            return "Opening your dashboard where you can track all your bookings and services.";
        }

        // Profile
        if (lowerQuery.includes("profile") || lowerQuery.includes("account") || lowerQuery.includes("my account") || lowerQuery.includes("settings")) {
            setTimeout(() => router.push("/dashboard"), 1000);
            return "Opening your profile page where you can manage your account settings.";
        }
        ...
        if (lowerQuery.includes("reward") || lowerQuery.includes("loyalty") || lowerQuery.includes("points") || lowerQuery.includes("gamification")) {
            ...
            return "Let me show you your loyalty rewards and points! You can redeem them for discounts and exclusive benefits.";
        }
        ...
        if (lowerQuery.includes("my car") || lowerQuery.includes("my vehicle") || lowerQuery.includes("add vehicle") || lowerQuery.includes("manage vehicle")) {
            ...
            return "Opening your vehicle management page where you can add and manage your cars.";
        }
        ...
        if (lowerQuery.includes("invoice") || lowerQuery.includes("bill") || lowerQuery.includes("receipt")) {
            ...
            return "Opening your invoices page where you can view and download all your payment receipts.";
        }
        ...
        if (lowerQuery.includes("notification") || lowerQuery.includes("alerts") || lowerQuery.includes("updates")) {
            ...
            return "Opening your notifications to show you all updates and alerts.";
        }
        ...
        if (lowerQuery.includes("referral") || lowerQuery.includes("refer") || lowerQuery.includes("invite friend")) {
            ...
            return "Opening your referral page! Refer friends and earn rewards for every successful referral.";
        }

        // ========== SERVICE INFORMATION ==========

        // Pricing
        if (matchesCategory(query, "pricing", settings.language)) {
            setTimeout(() => router.push("/booking"), 1000);
            return languageResponses[settings.language].pricingResponse;
        }

        // Service Duration
        if (lowerQuery.includes("how long") || lowerQuery.includes("duration") || lowerQuery.includes("time take")) {
            return "Service duration varies: Basic wash takes 30-45 minutes, interior cleaning takes 1-2 hours, full detailing takes 3-4 hours, and premium ceramic coating can take 6-8 hours for the best results.";
        }

        // Ceramic Coating
        if (lowerQuery.includes("ceramic") || lowerQuery.includes("coating") || lowerQuery.includes("protection")) {
            return "Our ceramic coating provides long-lasting protection with a brilliant shine! It lasts 2-5 years and protects against UV rays, chemicals, water spots, and minor scratches. It's like armor for your car's paint. Would you like to book this service?";
        }

        // Paint Protection Film (PPF)
        if (lowerQuery.includes("ppf") || lowerQuery.includes("paint protection film") || lowerQuery.includes("clear bra")) {
            return "Paint Protection Film is a transparent, thermoplastic urethane film that protects your car's paint from rock chips, scratches, and environmental damage. It's self-healing and virtually invisible. Perfect for new cars or high-end vehicles!";
        }

        // Detailing
        if (lowerQuery.includes("detail") || lowerQuery.includes("deep clean")) {
            return "Our full detailing service includes deep interior cleaning, exterior wash and wax, engine bay cleaning, tire dressing, and more. We'll make your car look brand new! It typically takes 3-4 hours.";
        }

        // Interior Cleaning
        if (lowerQuery.includes("interior") && (lowerQuery.includes("clean") || lowerQuery.includes("service"))) {
            return "Our interior cleaning includes vacuuming, steam cleaning, leather conditioning, dashboard polishing, and odor removal. We'll make your cabin spotless and fresh!";
        }

        // Headlight Restoration
        if (lowerQuery.includes("headlight") || lowerQuery.includes("foggy light") || lowerQuery.includes("yellow headlight")) {
            return "Headlight restoration removes yellowing and cloudiness, improving visibility and appearance. The process includes sanding, polishing, and UV-protective coating. Takes about 1 hour.";
        }
        ...
        if (lowerQuery.includes("fleet") || lowerQuery.includes("business") || lowerQuery.includes("commercial") || lowerQuery.includes("multiple cars")) {
            ...
            return "We offer special fleet services for businesses! Get volume discounts, priority scheduling, and dedicated account management. Let me show you our fleet packages.";
        }

        // ========== SUBSCRIPTIONS ==========

        if (lowerQuery.includes("subscription") || lowerQuery.includes("monthly plan") || lowerQuery.includes("package")) {
            return "We offer convenient subscription plans for regular car care! Choose from monthly or quarterly packages with unlimited washes, discounts on premium services, and priority booking. Would you like to see our plans?";
        }
        ...
        if (lowerQuery.includes("offer") || lowerQuery.includes("deal") || lowerQuery.includes("discount") || lowerQuery.includes("promo") || lowerQuery.includes("coupon")) {
            return "We have exciting offers running! Check our homepage for current promotions. New customers get 20% off their first service, and we have seasonal discounts too!";
        }
        ...
        if (lowerQuery.includes("review") || lowerQuery.includes("rating") || lowerQuery.includes("testimonial") || lowerQuery.includes("customer feedback")) {
            return "We have a 5-star rating on Google! Our customers love our attention to detail and premium service. You can read reviews on our website or check out video testimonials from happy customers.";
        }

        // ========== PAYMENT & BILLING ==========

        if (lowerQuery.includes("payment method") || lowerQuery.includes("how to pay") || lowerQuery.includes("pay")) {
            return "We accept all major payment methods: Credit/Debit cards, UPI (Google Pay, PhonePe, Paytm), Net Banking, and Digital Wallets. Payment is secure and processed after service completion.";
        }

        if (lowerQuery.includes("refund") || lowerQuery.includes("cancel") || lowerQuery.includes("money back")) {
            return "We have a flexible cancellation policy. If you cancel 24 hours before your appointment, you'll get a full refund. For cancellations within 24 hours, a small processing fee applies. Your satisfaction is our priority!";
        }
        ...
        if (lowerQuery.includes("open") || lowerQuery.includes("hours") || lowerQuery.includes("timing") || lowerQuery.includes("when") || lowerQuery.includes("schedule")) {
            return "We're open Monday to Friday 9 AM to 7 PM, Saturday 9 AM to 6 PM, and Sunday 10 AM to 5 PM. We're closed on public holidays. Would you like to book an appointment?";
        }

        if (lowerQuery.includes("location") || lowerQuery.includes("address") || lowerQuery.includes("where") || lowerQuery.includes("directions")) {
            setTimeout(() => router.push("/contact"), 1000);
            return "We're located at 123 Car Care Street, Auto District, Tirupur. I'll open the contact page with our exact location and Google Maps directions.";
        }

        if (lowerQuery.includes("whatsapp") || lowerQuery.includes("message") || lowerQuery.includes("chat")) {
            return "You can reach us on WhatsApp at +91 98765 43210. Just send us a message and we'll respond quickly! You can also use the WhatsApp button on our website.";
        }

        // ========== SUPPORT & FEEDBACK ==========

        if (matchesCategory(query, "help", settings.language)) {
            return languageResponses[settings.language].supportResponse;
        }

        if (lowerQuery.includes("feedback") || lowerQuery.includes("suggest") || lowerQuery.includes("complaint") || lowerQuery.includes("improve")) {
            return "Thank you for wanting to share feedback! Your input helps us improve. Would you like to submit feedback now? You can rate your experience and tell us your thoughts.";
        }

        // ========== CAR CARE ADVICE ==========

        if (lowerQuery.includes("maintain") || lowerQuery.includes("care for") || lowerQuery.includes("protect my car")) {
            return "Regular maintenance is key! We recommend washing every 2 weeks, waxing every 3 months, and ceramic coating every 2-3 years for best protection. Interior should be detailed every 6 months.";
        }

        if (lowerQuery.includes("wash") && (lowerQuery.includes("how often") || lowerQuery.includes("frequency"))) {
            return "For optimal care, wash your car every 1-2 weeks, especially if you drive in dusty conditions or park outdoors. Regular washing prevents dirt buildup and paint damage.";
        }

        // ========== SPECIFIC SERVICE QUERIES ==========

        if (lowerQuery.includes("polish") || lowerQuery.includes("buffing")) {
            return "Our polishing service removes minor scratches, swirl marks, and oxidation to restore your paint's shine. We use professional-grade compounds and machines for a perfect finish!";
        }

        if (lowerQuery.includes("wax") || lowerQuery.includes("sealant")) {
            return "Waxing provides a protective layer and deep shine. We use premium carnauba wax that lasts 2-3 months. For longer protection, consider our ceramic coating!";
        }

        if (lowerQuery.includes("engine") && lowerQuery.includes("clean")) {
            return "Engine bay detailing removes dirt, grease, and grime, making your engine look new and helping with maintenance inspections. It's safe for all modern engines with proper waterproofing.";
        }

        if (lowerQuery.includes("odor") || lowerQuery.includes("smell") || lowerQuery.includes("ac clean")) {
            return "We offer odor removal and AC sanitization! Using ozone treatment and enzymatic cleaners, we eliminate bad smells from smoke, pets, food, and mold. Your cabin will smell fresh again!";
        }
        ...
        if (lowerQuery.includes("luxury car") || lowerQuery.includes("premium car") || lowerQuery.includes("bmw") || lowerQuery.includes("mercedes") || lowerQuery.includes("audi")) {
            return "We specialize in luxury and premium vehicles! Our technicians are trained in handling high-end cars with extra care, using premium products specifically designed for luxury vehicles.";
        }

        if (lowerQuery.includes("bike") || lowerQuery.includes("motorcycle") || lowerQuery.includes("two wheeler")) {
            return "Yes, we service bikes and motorcycles too! We offer bike detailing, ceramic coating for bikes, and protection packages. Your two-wheeler deserves premium care too!";
        }

        if (lowerQuery.includes("suv") || lowerQuery.includes("large vehicle")) {
            return "We handle all vehicle sizes including SUVs, MUVs, and large vehicles. Pricing varies based on vehicle size, and we ensure thorough cleaning even for the biggest vehicles!";
        }
        ...
        if (lowerQuery.includes("how to book") || lowerQuery.includes("booking process")) {
            setTimeout(() => router.push("/booking"), 1000);
            return "Booking is easy! Just select your service, choose your vehicle type, pick a date and time slot, and confirm. You'll get instant confirmation and can track your booking in real-time!";
        }

        if (lowerQuery.includes("reschedule") || lowerQuery.includes("change appointment")) {
            return "You can reschedule your booking anytime from your dashboard. Just go to 'My Bookings', select the booking, and choose a new time slot. No charges for rescheduling!";
        }

        if (lowerQuery.includes("same day") || lowerQuery.includes("emergency") || lowerQuery.includes("urgent")) {
            return "We try to accommodate same-day bookings based on availability! Check our booking page for available slots today, or call us directly for urgent requests.";
        }

        // ========== QUALITY & GUARANTEE ==========

        if (lowerQuery.includes("guarantee") || lowerQuery.includes("warranty") || lowerQuery.includes("quality")) {
            return "We stand behind our work with a satisfaction guarantee! If you're not happy with any service, we'll redo it free of charge. Some services like ceramic coating come with multi-year warranties.";
        }

        if (lowerQuery.includes("certified") || lowerQuery.includes("trained") || lowerQuery.includes("professional")) {
            return "All our technicians are certified professionals with years of experience. We use only premium, branded products and follow international detailing standards for the best results.";
        }

        // ========== TECHNOLOGY & AI ==========

        if (lowerQuery.includes("ai") || lowerQuery.includes("technology") || lowerQuery.includes("shashti ai")) {
            return "We use AI-powered systems for service recommendations, real-time tracking, and smart scheduling. However, I'm Dinesh, your voice assistant - separate from Shashti AI. I help you navigate and find information!";
        }

        if (lowerQuery.includes("tracking") || lowerQuery.includes("live status") || lowerQuery.includes("progress")) {
            return "Yes! We have live service tracking. Once your service starts, you can see real-time progress with photos at each stage. It's like tracking a food delivery, but for your car!";
        }

        // ========== GENERAL HELP ==========

        if (lowerQuery.includes("what can you do") || lowerQuery.includes("capabilities") || lowerQuery.includes("features")) {
            return "I can help you navigate the website, book services, answer questions about our services, explain pricing, share our hours and location, help with support requests, collect feedback, and much more! Just ask me anything about Shashti Karz.";
        }

        if (lowerQuery.includes("who are you") || lowerQuery.includes("your name")) {
            return "I'm Dinesh, your friendly voice assistant for Shashti Karz! I'm here to help you navigate our services, answer questions, and make your car care experience smooth and easy. I can understand voice commands or you can type to me!";
        }

        if (lowerQuery.includes("thank") || lowerQuery.includes("thanks")) {
            return "You're very welcome! I'm always here to help. Is there anything else you'd like to know about our services?";
        }
        ...
        if (lowerQuery.includes("?") || lowerQuery.includes("what") || lowerQuery.includes("how") || lowerQuery.includes("when") || lowerQuery.includes("where") || lowerQuery.includes("why")) {
            return "I'd love to help answer your question! Could you please be more specific? You can ask me about our services, pricing, location, hours, booking process, or anything else about Shashti Karz.";
        }

        // Generic greeting
        if (lowerQuery.includes("hi") || lowerQuery.includes("hello") || lowerQuery.includes("hey")) {
            return "Hello! How can I assist you today? You can ask me about our services, book an appointment, track your order, check pricing, or get support.";
        }

        // Default response
        return "I'm here to help! You can ask me to navigate to different pages, book services, track your orders, check pricing, view our gallery, manage your profile, see rewards, or get support. What would you like to do?";
    };
                ...
                <div className="fixed bottom-6 left-6 z-50 w-[400px] h-[600px] bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-purple-500/30">
                    ...
                    <div className="bg-gradient-to-r from-purple-600 to-indigo-600 p-4 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-white font-bold text-lg">Dinesh</h3>
                                <p className="text-purple-100 text-xs">Your Voice Assistant</p>
                            </div>
                        </div>
